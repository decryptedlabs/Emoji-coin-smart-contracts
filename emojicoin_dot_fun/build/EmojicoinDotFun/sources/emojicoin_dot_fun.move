module emojicoin_dot_fun::emojicoin_dot_fun {
    ...
    const EMOJICOIN_NAME_SUFFIX: vector<u8> = b" movementcoin";
    const EMOJICOIN_STRUCT_NAME: vector<u8> = b"Movementcoin";
    const EMOJICOIN_LP_NAME_SUFFIX: vector<u8> = b" movementcoin LP";
    const EMOJICOIN_LP_STRUCT_NAME: vector<u8> = b"MovementcoinLP";
    ...
    struct LPCoinCapabilities<phantom Movementcoin, phantom MovementcoinLP> has key {
        burn: BurnCapability<MovementcoinLP>,
        mint: MintCapability<MovementcoinLP>,
    }
...
public entry fun register_market(
        ...
        RegistryAddress
    {
        ...
        let event = simulate_swap_inner<Movementcoin, MovementcoinLP>(
            swapper_address,
            input_amount,
            is_sell,
            integrator,
        ...
        if (is_sell) { // If selling, no possibility of state transition.
            ...
            // Transfer funds.
            coin::register<Movementcoin>(swapper); // For better feedback if insufficient funds.
            coin::transfer<Movementcoin>(swapper, market_address, input_amount);
            ...
            *local_cumulative_pool_fees_quote_ref_mut =
            ...
            if (results_in_state_transition) { // Buy with state transition.
                ...
                let lp_coins =
                    mint_lp_coins<Movementcoin, MovementcoinLP>(market_address, LP_TOKENS_INITIAL);
                coin::deposit<MovementcoinLP>(market_address, lp_coins);
    ...
    public entry fun chat<Movementcoin, MovementcoinLP>(
        user: &signer,
        market_address: address,
    ...
    // Verify chat message_bytes length.
    let message_length = vector::length(&message_bytes);
    assert!(message_length <= MAX_CHAT_MESSAGE_LENGTH, E_CHAT_MESSAGE_TOO_LONG);
    assert!(message_length > 0, E_CHAT_MESSAGE_EMPTY);

        let message: String = string::utf8(message_bytes);
        ...
        *local_cumulative_n_chat_messages = *local_cumulative_n_chat_messages + 1;
        ...
        let user_emojicoin_balance = if (!coin::is_account_registered<Movementcoin>(user_address)) {
            if (!account::exists_at(user_address)) {
                aptos_account::create_account(user_address);
            };
            coin::register<Movementcoin>(user);
            0
        } else {
            coin::balance<Movementcoin>(user_address)
        };
    ...
    public entry fun provide_liquidity<Movementcoin, MovementcoinLP>(
        provider: &signer,
        market_address: address,
        quote_amount: u64,
        ...
        assert_valid_coin_types<Movementcoin, MovementcoinLP>(market_address);
        ...
        // Transfer coins.
        coin::transfer<Movementcoin>(provider, market_address, event.base_amount);
        coin::transfer<AptosCoin>(provider, market_address, event.quote_amount);
        let lp_coins = mint_lp_coins<Movementcoin, MovementcoinLP>(
    ...
    public entry fun remove_liquidity<Movementcoin, MovementcoinLP>(
        provider: &signer,
        market_address: address,
        lp_coin_amount: u64,
        min_quote_out: u64,
        ...
        assert_valid_coin_types<Movementcoin, MovementcoinLP>(market_address);
        let provider_address = signer::address_of(provider);
        let event = simulate_remove_liquidity_inner<Movementcoin>(
            provider_address,
            lp_coin_amount,
            market_ref_mut,
        );
        ...
        coin::transfer<Movementcoin>(&market_signer, provider_address, base_total);
        ...
        let lp_coins = coin::withdraw<MovementcoinLP>(provider, event.lp_coin_amount);
        burn_lp_coins<Movementcoin, MovementcoinLP>(market_ref_mut.metadata.market_address, lp_coins);
...
//TODO: market_metadata_by_emoji_bytes change this to market_metadata_by_symbol_bytes
    ...
    #[view]
    public fun market_view<Movementcoin, MovementcoinLP>(market_address: address): MarketView
    acquires Market {
        ...
        MarketView {
            ...
            emojicoin_balance: coin::balance<Movementcoin>(market_address),
            emojicoin_lp_balance: coin::balance<MovementcoinLP>(market_address),
        }
    }
    ...
    #[view]
    public fun simulate_swap<Movementcoin, MovementcoinLP>(
        swapper: address,
        market_address: address,
        input_amount: u64,
        is_sell: bool,
    ...
    acquires Market {
        ...
        simulate_swap_inner<Movementcoin, MovementcoinLP>(
            swapper,
            input_amount,
            is_sell,
            integrator,
    ...
    #[view]
    public fun simulate_remove_liquidity<Movementcoin>(
        provider: address,
        market_address: address,
        lp_coin_amount: u64,
    ): Liquidity
    acquires Market {
        assert!(exists<Market>(market_address), E_NO_MARKET);
        simulate_remove_liquidity_inner<Movementcoin>(
            provider,
            lp_coin_amount,
            borrow_global(market_address),
        )
...
let integrator_fee = fee;
    ...
    fun simulate_swap_inner<Movementcoin, MovementcoinLP>(
        swapper: address,
        input_amount: u64,
        is_sell: bool,
        integrator: address,
        ...
        let balance_before = if (coin::is_account_registered<Movementcoin>(swapper)) {
            coin::balance<Movementcoin>(swapper)
        } else {
            0
        };
    ...
    inline fun assert_valid_coin_types<Movementcoin, MovementcoinLP>(market_address: address) {
        assert!(
            exists<LPCoinCapabilities<Movementcoin, MovementcoinLP>>(market_address),
    ...
    inline fun burn_lp_coins<Movementcoin, MovementcoinLP>(
        market_address: address,
        coin: Coin<MovementcoinLP>,
    ) acquires LPCoinCapabilities {
        let coin_caps = borrow_global<LPCoinCapabilities<Movementcoin, MovementcoinLP>>(market_address);
        coin::burn<MovementcoinLP>(coin, &coin_caps.burn);
    }
    ...
    inline fun ensure_coins_initialized<Movementcoin, MovementcoinLP>(
        market_ref: &Market,
        ...
        if (!exists<LPCoinCapabilities<Movementcoin, MovementcoinLP>>(market_address)) {
            ...
            let emojicoins = coin::mint<Movementcoin>(EMOJICOIN_SUPPLY, &mint_cap);
            ...
            let (burn_cap, freeze_cap, mint_cap) = coin::initialize<MovementcoinLP>(
                market_signer,
    ...
    inline fun mint_lp_coins<Movementcoin, MovementcoinLP>(
        market_address: address,
        amount: u64,
    ): Coin<MovementcoinLP> acquires LPCoinCapabilities {
        let coin_caps = borrow_global<LPCoinCapabilities<Movementcoin, MovementcoinLP>>(market_address);
        coin::mint<MovementcoinLP>(amount, &coin_caps.mint)
    }
    ...
    inline fun simulate_remove_liquidity_inner<Movementcoin>(
        provider: address,
        lp_coin_amount: u64,
        market_ref: &Market,
    ): Liquidity {
        ...
        let base_donation_claim_amount = coin::balance<Movementcoin>(market_address) - base_reserves;
    ...
    // debug::print<address>(&market_address);
    // debug::print<address>(&type_info::account_address(emoji_type));
    // debug::print<address>(&type_info::account_address(lp_type));
    // debug::print<vector<u8>>(&type_info::module_name(emoji_type));
    // debug::print<vector<u8>>(&type_info::module_name(lp_type));
    // debug::print<vector<u8>>(&type_info::struct_name(emoji_type));
    // debug::print<vector<u8>>(&type_info::struct_name(lp_type));
    // debug::print<vector<u8>>(&COIN_FACTORY_AS_BYTES);
    // debug::print<vector<u8>>(&EMOJICOIN_STRUCT_NAME);
    // debug::print<vector<u8>>(&EMOJICOIN_LP_STRUCT_NAME);

debug::print<address>(&market_address);
debug::print<address>(&type_info::account_address(emoji_type));
debug::print<address>(&type_info::account_address(lp_type));

debug::print<String>(&string::utf8(type_info::module_name(emoji_type)));
debug::print<String>(&string::utf8(type_info::module_name(lp_type)));   
debug::print<String>(&string::utf8(COIN_FACTORY_AS_BYTES));        
debug::print<String>(&string::utf8(type_info::struct_name(emoji_type)));
debug::print<String>(&string::utf8(EMOJICOIN_STRUCT_NAME)); 
debug::print<String>(&string::utf8(type_info::struct_name(lp_type)));
debug::print<String>(&string::utf8(EMOJICOIN_LP_STRUCT_NAME));  
    ...
    #[test_only] public fun assert_valid_coin_types_test_only<Movementcoin, MovementcoinLP>(
        market_address: address,
    ) {
        assert_valid_coin_types<Movementcoin, MovementcoinLP>(market_address)
    }
    ...
    #[test_only] public fun exists_lp_coin_capabilities<Movementcoin, MovementcoinLP>(
        market_address: address,
    ): bool {
        exists<LPCoinCapabilities<Movementcoin, MovementcoinLP>>(market_address)
    }
    ...
    #[test_only] public fun valid_coin_types_test_only<Movementcoin, MovementcoinLP>(
        market_address: address,
    ): bool {
        valid_coin_types<Movementcoin, MovementcoinLP>(market_address)
    }
